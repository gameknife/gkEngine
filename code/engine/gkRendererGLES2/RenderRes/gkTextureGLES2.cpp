...
struct gkTexture2DGLES2StreamingTask : public ITask
{
	...
	virtual void Execute() 
	{
		...
		loaderror = loader->Load_Data(pTextureFile->DataPtr() );
...
//					}
				...
				glTexImage2D(GL_TEXTURE_2D, 0, format, m_uWidth, m_uHeight, 0, format, type, 0);
			}
			else if ( itusage->second == _T("RAW") )
			{
				...
				glGenTextures(1, &m_uHwTexture2D);
				glBindTexture(GL_TEXTURE_2D, m_uHwTexture2D);
				glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, m_uWidth, m_uHeight, 0, GL_ALPHA, GL_UNSIGNED_BYTE, 0);
			}
...
// 	if ( getRenderer()->getDeviceContex()-> )
// 	{
// 	}
	...
	if( strLastSlash )
	{
		if ( !_tcsicmp( strLastSlash, _T(".raw") ) )
		{
			...
			if (m_uHeight != 0)
			{
				...
				gkLogMessage(_T("ADD_memcpycall"));
...
void gkTextureGLES2::RawDataFlush()
{
	...
	{
		glBindTexture( GL_TEXTURE_2D, m_uHwTexture2D );
		glTexImage2D( GL_TEXTURE_2D, 0, GL_ALPHA, m_uWidth, m_uHeight, 0, GL_ALPHA, GL_UNSIGNED_BYTE, m_rawData  );
