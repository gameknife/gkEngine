	  ...
	  msaaMaxSamples:(GLuint)maxSamples
{
    ...
    //bigframe.size.height = frame.size.width;
	...
	if((self = [super initWithFrame:frame])) {
        ...
        _loading_context = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES1 sharegroup:_context.sharegroup];
#elif BUILD_OGLES2
		_context = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2];
        _loading_context = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2 sharegroup:_context.sharegroup];
#endif
...
- (void) dealloc
{
    ...
    [_loading_context release];
    _loading_context = nil;
...
{
	if(![EAGLContext setCurrentContext:_loading_context]) {
		printf("Failed to set current context %p in %s\n", _loading_context, __FUNCTION__);
	}
}
