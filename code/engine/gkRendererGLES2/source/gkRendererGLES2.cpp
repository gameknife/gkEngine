...
IResourceManager*	gkRendererGLES2::m_pTextureManager = NULL;
IResourceManager*	gkRendererGLES2::m_pMeshManager = NULL;
IResourceManager*	gkRendererGLES2::m_pMaterialManager = NULL;
IResourceManager*	gkRendererGLES2::m_pShaderManager = NULL;

gkAuxRendererGLES2*		gkRendererGLES2::m_pAuxRenderer = 0;

gkShaderParamDataSource	gkRendererGLES2::m_ShaderParamDataSource;

gkRendererGLES2* getRenderer()
{
	return static_cast<gkRendererGLES2*>(gEnv->pRenderer);
}


gkRendererGLES2::gkRendererGLES2( void )
{
...
HWND gkRendererGLES2::Init( ISystemInitInfo& sii )
{
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8_OES, GetScreenWidth(), GetScreenHeight());

	return m_hWnd;
}

void gkRendererGLES2::RT_CleanRenderSequence()
{
...
void gkRendererGLES2::RT_SkipOneFrame(int framecount)
{
	m_bSkipThisFrame = framecount;
	RT_CleanRenderSequence();
}

void gkRendererGLES2::FX_ColorGradingTo( gkTexturePtr& pCch, float fAmount )
{
	//throw std::exception("The method or operation is not implemented.");
}

bool gkRendererGLES2::RT_EndRender()
{
...
bool gkRendererGLES2::RT_StartRender()
{
	...
	if (m_bSkipThisFrame == 0)
	{
		...
		//FX_PushHwDepthTarget( gkTextureManager::ms_SceneDepth );
...
void gkRendererGLES2::SetCurrContent( HWND hWnd, uint32 posx, uint32 posy, uint32 width, uint32 height, bool fullscreen  )
{
...
uint32 gkRendererGLES2::GetScreenHeight(bool forceOrigin)
{
	return m_uScreenHeight;
}

uint32 gkRendererGLES2::GetScreenWidth(bool forceOrigin)
{
	return m_uScreenWidth;
}


IRenderSequence* gkRendererGLES2::RT_GetRenderSequence()
{
	return m_pUpdatingRenderSequence;
}

IRenderSequence* gkRendererGLES2::RT_SwapRenderSequence()
{
...
void gkRendererGLES2::RC_SetMainCamera( const CCamera* cam )
{
	m_ShaderParamDataSource.setMainCamera(cam);
}

void gkRendererGLES2::_render( const gkRenderOperation& op, bool isShadowPass /*= false*/ )
{
	...
	case gkRenderOperation::OT_TRIANGLE_LIST:
		{
			...
			if (gkGLExtension::EXT_VAO)
			{
				...
				{
					gkGLExtension::glGenVertexArraysOES(1, &vaoName);
					gkGLExtension::glBindVertexArrayOES( vaoName );
...
IResourceManager* gkRendererGLES2::getResourceManager( BYTE type )
{
	switch (type)
	{
...
void gkRendererGLES2::RP_ProcessZpassObjects( const gkRenderableList* objs, IShader* pShader, bool skined /*= 0*/ )
{
	pShader->FX_Begin( 0, 0 );
...
void gkRendererGLES2::RP_ProcessRenderLayer( uint8 layerID, bool skined, DWORD systemMacro )
{
...
void gkRendererGLES2::RP_ProcessShaderGroup( const gkShaderGroup* pShaderGroup, bool skined )
{
	...
	// global cam param
	Matrix44 viewiMat = gkRendererGLES2::getShaderContent().getInverseViewMatrix();
	...
	pShader->FX_SetFloat3("g_camDir", gkRendererGLES2::getShaderContent().getCamDir());
	pShader->FX_SetFloat3("g_camPos", gkRendererGLES2::getShaderContent().getCamPos());
...
void gkRendererGLES2::RP_ProcessRenderList( std::list<gkRenderable*>& renderableList, gkShaderGLES2* pShader, int shadowcascade )
{
	// 渲染list中的每一个renderable
	std::list<gkRenderable*>::const_iterator iter, iterend;
	...
	for (iter = renderableList.begin(); iter != iterend; ++iter)
	{
		...
		Matrix44 mvpMat = gkRendererGLES2::getShaderContent().getWorldViewProjMatrix();
		Matrix44 worldMat = gkRendererGLES2::getShaderContent().getWorldMatrix();
		switch( shadowcascade )
		{
		case 0:
			mvpMat = gkRendererGLES2::getShaderContent().getWorldViewProjMatrix_ShadowCascade(0);
			worldMat = gkRendererGLES2::getShaderContent().getWorldViewMatrix_ShadowCascade(0);
			break;
		case 1:
			mvpMat = gkRendererGLES2::getShaderContent().getWorldViewProjMatrix_ShadowCascade(1);
			worldMat = gkRendererGLES2::getShaderContent().getWorldViewMatrix_ShadowCascade(1);
			break;
		case 2:
			mvpMat = gkRendererGLES2::getShaderContent().getWorldViewProjMatrix_ShadowCascade(2);
			worldMat = gkRendererGLES2::getShaderContent().getWorldViewMatrix_ShadowCascade(2);
			break;
		...
		pShader->FX_SetMatrix("g_matWorldI", gkRendererGLES2::getShaderContent().getInverseWorldMatrix());
...
Ray gkRendererGLES2::GetRayFromScreen( uint32 nX, uint32 nY )
{
	Ray ret;
...
Vec3 gkRendererGLES2::ProjectScreenPos( const Vec3& worldpos )
{
	Vec3 ret;
...
Vec2i gkRendererGLES2::GetWindowOffset()
{
	return Vec2i(0,0);
}

void gkRendererGLES2::SetRenderState( uint32 key, uint32 value )
{

}

void gkRendererGLES2::RC_SetSunDir( const Vec3& lightdir )
{
	m_ShaderParamDataSource.setLightDir(lightdir);
}

void gkRendererGLES2::FX_PushRenderTarget( uint8 channel, gkTexturePtr src, gkTexturePtr depth, bool bNeedStencil /*= false*/ )
{
...
void gkRendererGLES2::FX_DrawScreenQuad()
{
	FX_DrawScreenQuad(Vec4(0,0,1,1));
}

void gkRendererGLES2::FX_DrawScreenQuad( Vec4 region )
{
	glDisable(GL_DEPTH_TEST);
	glDisable(GL_CULL_FACE);
	...
	if (tmpVBO == 0) {
		...
		glBufferData(GL_ARRAY_BUFFER, 28 * sizeof(float), 0, GL_DYNAMIC_DRAW);
	}
	...
	{
		//getRenderer()->getShaderContent().getMainCamera()
		struct STmpVert
		{
			Vec2 pos;
			Vec2 tex;
		...
		glBufferData(GL_ARRAY_BUFFER, 28 * sizeof(float), verts, GL_DYNAMIC_DRAW);
	}
...
void gkRendererGLES2::RC_SetShadowCascadeCamera( const CCamera* cam, uint8 index )
{
...
void gkRendererGLES2::FX_TexBlurGaussian( gkTexturePtr tgt, int nAmount, float fScale, float fDistribution, gkTexturePtr tmp, int iterate /*= 1*/ )
{
	// blur first
...
IDeviceRenderContext* gkRendererGLES2::getDeviceContex()
{
	return m_pDeviceContext;
}

Vec3 gkRendererGLES2::ScreenPosToViewportPos(Vec3 screenPos)
{
...
void gkRendererGLES2::apply_vertex_layout(const gkRenderOperation &op)
{
