...
{
	IMaterial* mat = pRend->getMaterial();


	if (g_pRendererCVars->r_defaultmat)
	{
		pShader = gkMaterialManager::ms_DefaultMaterial->getShader().getPointer();
		mat = gkMaterialManager::ms_DefaultMaterial.getPointer();
	}
	...
	// 插入 renderable
	i->second.addRenderable(pRend, mat);
}
...
void gkShaderGroup::clear()
{
	...
	for (i = m_opaqueMaterialMap.begin(); i != iend; ++i)
	{
		...
		i->second.m_vecRenderable_Skinned.clear();
	}

	//////////////////////////////////////////////////////////////////////////
	// MATERIAL 是可能被删除的，这里要清除 [3/30/2013 Kaiming]
